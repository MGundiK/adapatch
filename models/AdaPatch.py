"""
AdaPatch model with Hydra post-fusion support.

New args (all backward-compatible with defaults):
  --cv_post_fusion       Enable post-fusion cross-variable mixing (flag)
  --cv_post_fusion_mode  Mode: 'hydra_gated', 'hydra', 'mlp' (default: same as cv_mode)
  --cv_post_fusion_rank  Rank for post-fusion mixer (default: same as cv_rank)

Recommended configs:
  High-var (C>=20):  --cv_post_fusion --cv_post_fusion_mode hydra_gated --cv_post_fusion_rank <pred_len>
  Low-var  (C<20):   No post-fusion (default)
"""

import torch
import torch.nn as nn

from layers.learnable_ema import LearnableEMA
from layers.network import AdaPatchNetwork
from layers.revin import RevIN


class Model(nn.Module):
    def __init__(self, configs):
        super().__init__()

        seq_len = configs.seq_len
        pred_len = configs.pred_len
        c_in = configs.enc_in

        # RevIN
        self.revin = getattr(configs, 'revin', 1)
        self.revin_layer = RevIN(c_in, affine=True, subtract_last=False)

        # Learnable EMA decomposition
        alpha_init = getattr(configs, 'alpha', 0.3)
        ema_reg = getattr(configs, 'ema_reg_lambda', 0.0)
        ema_backend = getattr(configs, 'ema_backend', 'matrix')
        self.decomp = LearnableEMA(
            num_features=c_in,
            alpha_init=alpha_init,
            backend=ema_backend,
            reg_lambda=ema_reg
        )

        # Network
        patch_len = getattr(configs, 'patch_len', 16)
        stride = getattr(configs, 'stride', 8)
        padding_patch = getattr(configs, 'padding_patch', 'end')
        n_blocks = getattr(configs, 'n_blocks', 1)
        kernel_sizes = getattr(configs, 'kernel_sizes', (3, 5, 7))
        agg_kernel = getattr(configs, 'agg_kernel', 5)

        # Cross-variable mixing args
        cv_mode = getattr(configs, 'cv_mixing', 'none')
        cv_rank = getattr(configs, 'cv_rank', 32)

        # Post-fusion args (new)
        cv_post_fusion = getattr(configs, 'cv_post_fusion', False)
        cv_post_fusion_mode = getattr(configs, 'cv_post_fusion_mode', None)
        cv_post_fusion_rank = getattr(configs, 'cv_post_fusion_rank', None)

        self.net = AdaPatchNetwork(
            seq_len=seq_len, pred_len=pred_len,
            patch_len=patch_len, stride=stride,
            padding_patch=padding_patch,
            n_blocks=n_blocks,
            kernel_sizes=kernel_sizes,
            agg_kernel=agg_kernel,
            use_multiscale=getattr(configs, 'use_multiscale', True),
            use_causal=getattr(configs, 'use_causal', True),
            use_gated_fusion=getattr(configs, 'use_gated_fusion', False),
            use_agg_conv=getattr(configs, 'use_agg_conv', True),
            n_vars=c_in,
            cv_mode=cv_mode,
            cv_rank=cv_rank,
            cv_kernel=getattr(configs, 'cv_kernel', 7),
            cv_depth=getattr(configs, 'cv_depth', 1),
            cv_post_pw=getattr(configs, 'cv_post_pw', False),
            # Post-fusion
            cv_post_fusion=cv_post_fusion,
            cv_post_fusion_mode=cv_post_fusion_mode,
            cv_post_fusion_rank=cv_post_fusion_rank,
        )

    def forward(self, x):
        if self.revin:
            x = self.revin_layer(x, 'norm')

        seasonal, trend = self.decomp(x)
        out = self.net(seasonal, trend)

        if self.revin:
            out = self.revin_layer(out, 'denorm')

        return out

    def get_ema_reg_loss(self):
        return self.decomp.regularization_loss()

    def get_alpha_values(self):
        return self.decomp.alpha.detach().cpu().numpy()
